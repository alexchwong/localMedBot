...
class HTTPRegressionTests(unittest.TestCase):
    def test_auth_shape_and_redirect_rejection(self):
        with endpoint([reply({'ok':True})]) as (url,requests):
            settings={'model':'fixture','temperature':0,'max_tokens':50,'timeout_seconds':1,'json_mode':True}
        ...
        with endpoint([401]) as (url,_):
            settings={'model':'fixture','temperature':0,'max_tokens':5,'timeout_seconds':1,'json_mode':True}
    ...
    def test_http_pipeline_does_not_persist_secret(self):
        with TemporaryDirectory() as td:
            ...
            for node in ['facts','facts_check','draft','draft_check']:
                ...
                rows.append(lambda request,template=template: reply(resolve(template,json.loads(request['messages'][-1]['content']))))
